...
```bash
git clone https://github.com/Shawn-TV/Project-Locus.git
cd Project-Locus
```
...
python tools/make_submission.py \
  ...
  --version V1 \
  --code-repo https://github.com/Shawn-TV/Project-Locus \
  --out-dir build/V1 \
...
Project Locus preserves MolParser E-SMILES captions whenever possible, because the competition expects Extended-SMILES syntax rather than plain SMILES only. The merger script also ensures populated rows include `<sep>` and keeps E-SMILES extension tags such as atom labels.
...
Project Locus is released under the MIT License. Third-party tools remain under their own licenses and terms:
...
Project Locus 是一个开源的分子结构图识别（OCSR）和 E-SMILES 提交包生成流程。它是为 [Bohrium 科学数据标注大赛赛道 1](https://www.bohrium.com/competitions/53859761357?tab=introduce) 准备的：赛题要求把每张分子图片识别成 Extended-SMILES，并按平台格式提交。
...
Project Locus 会尽量保留 MolParser 返回的 E-SMILES caption，因为比赛要求的是 Extended-SMILES，不只是普通 SMILES。合并脚本也会确保非空结果带有 `<sep>`，并保留原子标签等 E-SMILES 扩展标记。

缩写基团、Markush 标注、复杂桥环、立体化学和扫描图里的文字标签仍然可能需要人工复核。审计脚本就是为了这个循环设计的：回画、对比、检查、修正、重新打包。

## 第三方工具与许可证

Project Locus 使用 MIT License 开源。第三方工具仍然遵循它们自己的许可证和使用条款：
